...
static int audio_element_obu(void *s, IAMFContext *c, AVIOContext *pb, int len)
{
    ...
    for (int i = 0; i < num_parameters; i++) {
        ...
        uint8_t seen_param_types[3] = {0};
        static const char *param_names[] = { "MIX_GAIN", "DEMIXING", "RECON_GAIN" };
        type = ffio_read_leb(pbc);
        if (type >= 3 || seen_param_types[type]) {
            av_log(s, AV_LOG_ERROR, "Duplicate or invalid parameter type %u (%s) in audio element %u\n",
                   type, (type < 3 ? param_names[type] : "UNKNOWN"), audio_element_id);
            ret = AVERROR_INVALIDDATA;
            goto fail;
        }
        seen_param_types[type] = 1;

        if (type == AV_IAMF_PARAMETER_DEFINITION_MIX_GAIN) {
            av_log(s, AV_LOG_ERROR, "MIX_GAIN parameters are not allowed in audio elements\n");
            ret = AVERROR_INVALIDDATA;
            goto fail;
