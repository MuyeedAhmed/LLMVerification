...
int ff_request_frame(AVFilterLink *link)
{
    ...
    if (link->srcpad->request_frame)
        ...
        return AVERROR(ENOSYS);  // No request_frame and no upstream input
}
...
int ff_poll_frame(AVFilterLink *link)
{
    ...
    for (i = 0; i < link->src->nb_inputs; i++) {
        int val;
        if (!link->src->inputs[i]) return AVERROR(EPIPE);  // Broken input stream
                                   ...
                                   const char *args)
{
    ...
    while (*p) {
        ...
        if (!o) {
            ...
            return AVERROR(EINVAL);  // Too many unnamed options
        }
