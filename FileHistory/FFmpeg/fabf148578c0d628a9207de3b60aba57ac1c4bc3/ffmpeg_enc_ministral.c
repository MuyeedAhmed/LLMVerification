...
static int enc_open(void *opaque, const AVFrame *frame)
{
    ...
    case AVMEDIA_TYPE_VIDEO: {
        ...
        for (int i = 0; i < frame->nb_side_data; i++) {
            AVPacketSideData *sd = frame->side_data[i];
            if (sd->type == AV_PKT_DATA_DISPLAYMATRIX) {
                // Only promote the first displaymatrix side data to global
                // if it's not already set
                if (enc_ctx->color_range == AVCOL_RANGE_UNSPECIFIED) {
                    enc_ctx->color_range = av_display_rotation_get((int32_t *)sd->data);
                }
            } else {
