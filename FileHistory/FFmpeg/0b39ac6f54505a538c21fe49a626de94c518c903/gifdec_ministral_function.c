...
static int gif_read_image(GifState *s, AVFrame *frame)
{
    ...
    /* Fix for CVE-2014-8547: Ensure we don't write past the end of the frame */
    if (is_interleaved) {
        if (pass == 0 || pass == 1) {
            y1 = pass ? 2 : 4;
            ptr = ptr1 + linesize * y1;
        ...
        /* Ensure we don't write past the end of the frame */
        if (ptr + linesize > frame->data[0] + frame->linesize[0] * frame->height) {
            return AVERROR_INVALIDDATA;
        }
    }
