...
int ff_mp4_read_dec_config_descr(AVFormatContext *fc, AVStream *st, AVIOContext *pb)
{
    ...
    if (tag == MP4DecSpecificDescrTag) {
        ...
        if (st->codecpar->codec_id == AV_CODEC_ID_AAC) {
            ...
            // Preserve known channel layout from AAC decSpecificInfo
            if (cfg.channel_config) {
                st->codecpar->ch_layout.nb_channels = cfg.channel_config;
                st->codecpar->ch_layout.order = AV_CHANNEL_ORDER_NATIVE;
                st->codecpar->ch_layout.u.mask = av_get_channel_layout_native(cfg.channel_config);
            }
        }
    }
    return 0;
