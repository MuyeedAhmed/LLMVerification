                                           ...
                                           int partition_count)
{
    ...
    for (p = 0, j = 0; j < ch_used; ++j) {
        if (!do_not_decode[j]) {
            ...
            if (temp < 0 || temp >= ff_vorbis_nth_root(vr->classifications, codebook->dimensions)) {
                av_log(vc->avctx, AV_LOG_ERROR,
                       "Invalid vlc code %d decoding channel %d (max allowed: %d)\n",
                       temp, j, ff_vorbis_nth_root(vr->classifications, codebook->dimensions) - 1);
                return AVERROR_INVALIDDATA;
            }
                                     codebook->nb_bits, 3);
