...
static int check_pkt(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (pkt->dts < ref || duration >= INT_MAX) {
        av_log(s, AV_LOG_WARNING,
               "Adjusted DTS from %"PRId64" to %"PRId64" due to out-of-range duration (%"PRId64") for mov/mp4 format\n",
               pkt->dts, ref + 1, duration);
        pkt->dts = ref + 1;
        pkt->pts = AV_NOPTS_VALUE;
    }

    if (pkt->duration < 0 || pkt->duration > INT_MAX) {
        av_log(s, AV_LOG_WARNING, "Invalid duration %"PRId64" for mov/mp4 format\n", pkt->duration);
        return AVERROR(EINVAL);
    }
    return 0;
}
