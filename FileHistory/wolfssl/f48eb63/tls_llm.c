                          ...
                          byte msgType)
{
 ...
 /* Validate that the cookie matches the one sent in HelloRetryRequest */
 cookie = (Cookie*)extension->data;
 if (cookie == NULL || cookie->len != len || XMEMCMP(cookie->data, input + idx, len) != 0) {
     WOLFSSL_ERROR_MSG("Cookie mismatch or missing in new ClientHello");
     return HRR_COOKIE_ERROR;
 }
OSE(HRR_COOKIE_ERROR);
         return HRR_COOKIE_ERROR;
     }
 }
