...
{
    int err = MP_OKAY;
    volatile unsigned int i; // Made volatile to avoid compiler optimization issues
    volatile int j;          // Made volatile to avoid compiler optimization issues
    volatile unsigned int k; // Made volatile to avoid compiler optimization issues
#if defined(WOLFSSL_SMALL_STACK) && !defined(WOLFSSL_SP_NO_MALLOC)
    sp_int_digit* t = NULL;
#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901L) && \
    !defined(WOLFSSL_SP_NO_DYN_STACK)
