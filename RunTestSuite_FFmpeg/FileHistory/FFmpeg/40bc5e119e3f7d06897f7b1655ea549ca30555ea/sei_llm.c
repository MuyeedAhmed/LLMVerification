...
#include "libavutil/common.h"  // for av_mod_uintp2
...
{
    h->present = 1;
    h->cancel_flag = s->do_cancel_flag;
    if (!h->cancel_flag) {
        h->hflip = s->do_hor_flip;
        h->vflip = s->do_ver_flip;
        h->anticlockwise_rotation = av_mod_uintp2(s->do_anticlockwise_rotation, 16);
        h->persistence_flag = s->do_persistence_flag;
    }
    return 0;
}
