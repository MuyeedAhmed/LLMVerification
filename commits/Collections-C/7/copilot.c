enum cc_stat hashtable_new_conf(HashTableConf const * const conf, HashTable **out)
{
    ...
    table->buckets = conf->mem_calloc(table->capacity, sizeof(TableEntry*)); // Allocate only for pointers
...
static enum cc_stat resize(HashTable *t, size_t new_capacity)
{
    ...
    TableEntry **new_buckets = t->mem_calloc(new_capacity, sizeof(TableEntry*)); // Allocate only for pointers
