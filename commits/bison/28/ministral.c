...
grammar_rule_check_and_complete (symbol_list *r)
{
      ...
      if (first_rhs)
        {
          ...
          if (!UNIQSTR_EQ (lhs_type, rhs_type))
            ...
            {
              const bool is_cxx = STREQ (language->language, "c++")
  ...
  /* Generate actions for start symbols */
  if (r->content.sym == startsymbol)
    {
      /* Use the same obstack as scan-code */
      obstack_ptr = &scan_code_obstack;

      /* Generate the start rule action */
      char *action = obstack_alloc (obstack_ptr, 1024);
      snprintf (action, 1024, "{ %s = $1; }", startsymbol->tag);

      /* Initialize the action properties */
      code_props_rule_action_init (&r->action_props, action, r->rhs_loc, r,
                                   /* name */ NULL,
                                   /* type */ NULL,
                                   /* is_predicate */ false);
