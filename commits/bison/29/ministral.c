  ...
  if (nuseless_nonterminals || nuseless_productions)
    {
      ...
      /* Check if start symbol derives any sentence */
      if (!bitset_test (N, accept->content->number - ntokens))
        complain (&startsymbol_loc, fatal,
                  _("start symbol %s does not derive any sentence"),
                  startsymbol->tag);

      /* Check all start symbols (not just the main one) */
      for (symbol_number i = ntokens; i < nsyms; i++)
        {
          if (symbols[i]->content->status == start)
            {
              if (!bitset_test (N, i - ntokens))
                complain (&symbols[i]->location, fatal,
                          _("start symbol %s does not derive any sentence"),
                          symbols[i]->tag);
            }
        }
