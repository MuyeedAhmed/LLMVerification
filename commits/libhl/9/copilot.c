...
// Check if the table needs to grow and ensure growth is not already in progress
if (!__sync_fetch_and_add(&table->growing, 0) &&
    ht_count(table) > __sync_fetch_and_add(&table->size, 0) + HT_GROW_THRESHOLD &&
    (!table->max_size || __sync_fetch_and_add(&table->size, 0) < table->max_size))
{
    __sync_bool_compare_and_swap(&table->growing, 0, 1); // Mark growth as in progress
    ht_grow_table(table);
    __sync_bool_compare_and_swap(&table->growing, 1, 0); // Mark growth as complete
}
