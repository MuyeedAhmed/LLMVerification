...
unsigned int
my_vsf_sysutil_strlen(const char* p_text)
{
...
int
my_vsf_sysutil_retval_is_error(int retval)
{
  if (retval < 0)
  {
    return 1;
...
int
my_str_isempty(const struct mystr* p_str)
{
  return (p_str->len == 0);
}
...
/* Public functions */
void
my_str_alloc_text(struct mystr* p_str, const char* p_src)
{
  unsigned int len = my_vsf_sysutil_strlen(p_src);
  my_private_str_alloc_memchunk(p_str, p_src, len);
}

const char*
my_str_getbuf(const struct mystr* p_str)
{
  if (p_str->p_buf == 0)
  {
    ...
    my_private_str_alloc_memchunk((struct mystr*)p_str, 0, 0);
  }
  return p_str->p_buf;
}
...
static void
my_vsf_sysutil_alloc_statbuf(struct vsf_sysutil_statbuf** p_ptr)
{
  if (*p_ptr == NULL)
  {
    *p_ptr = my_vsf_sysutil_malloc(sizeof(struct stat));
  }
}

int
my_vsf_sysutil_stat(const char* p_name, struct vsf_sysutil_statbuf** p_ptr)
{
  my_vsf_sysutil_alloc_statbuf(p_ptr);
  return stat(p_name, (struct stat*) (*p_ptr));
}

int
my_str_stat(const struct mystr* p_str, struct vsf_sysutil_statbuf** p_ptr)
{
  return my_vsf_sysutil_stat(my_str_getbuf(p_str), p_ptr);
}

void
my_private_str_append_memchunk(struct mystr* p_str, const char* p_src,
                            unsigned int len)
{
  unsigned int buf_needed;
  ...
  if (buf_needed > p_str->alloc_bytes)
  {
    p_str->p_buf = my_vsf_sysutil_realloc(p_str->p_buf, buf_needed);
    p_str->alloc_bytes = buf_needed;
  }
  my_vsf_sysutil_memcpy(p_str->p_buf + p_str->len, p_src, len);
  p_str->p_buf[p_str->len + len] = '\0';
  p_str->len += len;
}

void
my_str_append_text(struct mystr* p_str, const char* p_src)
{
  unsigned int len = vsf_sysutil_strlen(p_src);
  my_private_str_append_memchunk(p_str, p_src, len);
}

const char*
my_vsf_sysutil_ulong_to_str(unsigned long the_ulong)
{
...
void
my_str_append_ulong(struct mystr* p_str, unsigned long the_ulong)
{
  my_str_append_text(p_str, my_vsf_sysutil_ulong_to_str(the_ulong));
}

void
my_str_append_char(struct mystr* p_str, char the_char)
{
  my_private_str_append_memchunk(p_str, &the_char, sizeof(the_char));
}

int
my_memcmp(const void* p_src1, const void* p_src2, unsigned int size)
{
  /* Safety */
  if (size == 0)
  {
...
static int
my_str_equal_internal(const char* p_buf1, unsigned int buf1_len,
                   const char* p_buf2, unsigned int buf2_len)
{
  ...
  retval = my_memcmp(p_buf1, p_buf2, minlen);
...
int
my_str_strcmp(const struct mystr* p_str1, const struct mystr* p_str2)
{
  return my_str_equal_internal(p_str1->p_buf, p_str1->len,
                            p_str2->p_buf, p_str2->len);
}

int
my_str_equal(const struct mystr* p_str1, const struct mystr* p_str2)
{
  return (my_str_strcmp(p_str1, p_str2) == 0);
}
...
get_unique_filename(struct mystr* p_outstr, const struct mystr* p_base_str)
{
  ...
  int retval;
  if (my_str_isempty(p_real_base_str))
  {
    my_str_alloc_text(&s_stou_str, "STOU");
    p_real_base_str = &s_stou_str;
  }
  else
  {
    /* Do not add any suffix at all if the name is not taken. */
    retval = my_str_stat(p_real_base_str, &s_p_statbuf);
    if (my_vsf_sysutil_retval_is_error(retval))
    {
       my_str_copy(p_outstr, p_real_base_str);
       return;
    }
  }
  while (1)
  {
    my_str_copy(p_outstr, p_real_base_str);
    my_str_append_char(p_outstr, '.');
    my_str_append_ulong(p_outstr, suffix);
    retval = my_str_stat(p_outstr, &s_p_statbuf);
    if (my_vsf_sysutil_retval_is_error(retval))
    {
      return;
    }
...
static void chatgpt_get_unique_filename(struct mystr* p_outstr, const struct mystr* p_base_str)
{
    static struct vsf_sysutil_statbuf* s_p_statbuf;
    unsigned int suffix = 1;
    struct mystr base_with_prefix;

    if (my_str_isempty(p_base_str)) {
        // STOU with no filename, use "STOU.<suffix>"
        my_str_alloc_text(&base_with_prefix, "STOU");
    } else {
        // Copy the original base string
        my_str_copy(&base_with_prefix, p_base_str);
    }

    my_str_append_char(&base_with_prefix, '.');
    
    int retval = my_str_stat(&base_with_prefix, &s_p_statbuf);
    if (my_vsf_sysutil_retval_is_error(retval))
    {
        my_str_copy(p_outstr, &base_with_prefix);
        return;
    }
...
int main() {
  int MAX_STR_LEN = 16;
  struct mystr base_str, out_str;
  char* base_buf = (char *)malloc(MAX_STR_LEN);
  char* out_buf = (char *)malloc(MAX_STR_LEN);

  klee_make_symbolic(base_buf, MAX_STR_LEN, "base_buf");
  base_buf[sizeof(base_buf) - 1] = '\0';
  out_buf[sizeof(out_buf) - 1] = '\0';
  
  base_str.PRIVATE_HANDS_OFF_p_buf = base_buf;
  base_str.PRIVATE_HANDS_OFF_len = MAX_STR_LEN-1;
  base_str.PRIVATE_HANDS_OFF_alloc_bytes = MAX_STR_LEN;

  out_str.PRIVATE_HANDS_OFF_p_buf = out_buf;
  out_str.PRIVATE_HANDS_OFF_len = MAX_STR_LEN-1;
  out_str.PRIVATE_HANDS_OFF_alloc_bytes = MAX_STR_LEN;

  get_unique_filename(&out_str, &base_str);

  // chatgpt individual test below

  // struct mystr base_str2, out_str2;
  // char* base_buf2 = (char *)malloc(MAX_STR_LEN);
  // char* out_buf2 = (char *)malloc(MAX_STR_LEN);

  // klee_make_symbolic(base_buf2, MAX_STR_LEN, "base_buf2");
  // klee_make_symbolic(out_buf2, MAX_STR_LEN, "out_buf2");
  // base_buf2[sizeof(base_buf2) - 1] = '\0';
  // out_buf2[sizeof(out_buf2) - 1] = '\0';

  // base_str2.PRIVATE_HANDS_OFF_p_buf = base_buf2;
  // base_str2.PRIVATE_HANDS_OFF_len = MAX_STR_LEN-1;
  // base_str2.PRIVATE_HANDS_OFF_alloc_bytes = MAX_STR_LEN;

  // out_str2.PRIVATE_HANDS_OFF_p_buf = out_buf2;
  // out_str2.PRIVATE_HANDS_OFF_len = MAX_STR_LEN-1;
  // out_str2.PRIVATE_HANDS_OFF_alloc_bytes = MAX_STR_LEN;

  // chatgpt_get_unique_filename(&out_str2, &base_str2);

  // comparison test
  struct mystr base_str2, out_str2;
  char* base_buf2 = (char *)malloc(MAX_STR_LEN);
  char* out_buf2 = (char *)malloc(MAX_STR_LEN);
  strcpy(base_buf2, base_buf);
  strcpy(out_buf2, out_buf);
  ...
  klee_assert(my_str_equal(&out_str, &out_str2));
