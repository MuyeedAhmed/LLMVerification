...
int main() {
  struct vsf_session session;
  struct mystr password;
  char* user_buf = (char *) malloc(VSFTP_USERNAME_MAX);
  char* pass_buf = (char *) malloc(VSFTP_PASSWORD_MAX);

  klee_make_symbolic(user_buf, VSFTP_USERNAME_MAX, "user_buf");
  klee_make_symbolic(pass_buf, VSFTP_USERNAME_MAX, "pass_buf");

  user_buf[VSFTP_USERNAME_MAX - 1] = '\0';
  pass_buf[VSFTP_PASSWORD_MAX - 1] = '\0';

  str_alloc_text(&session.user_str, user_buf);
  str_alloc_text(&password, pass_buf);

  enum EVSFPrivopLoginResult result = vsf_privop_do_login(&session, &password);
